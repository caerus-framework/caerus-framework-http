...
func (b *Bind) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &ss); err != nil {
		return fmt.Errorf("cf_http: bind must be a string or array of host:port: %w", err)
	}
	return b.set(ss)
}
...
func parseBindList(ss []string) (Bind, error) {
	if len(ss) == 0 {
		return nil, fmt.Errorf("cf_http: bind array must not be empty")
	}
	...
	for _, raw := range ss {
		...
		if s == "" {
			return nil, fmt.Errorf("cf_http: bind entry is empty")
		}
		host, port, err := net.SplitHostPort(s)
		if err != nil {
			return nil, fmt.Errorf("cf_http: bind %q: want host:port (empty host is all interfaces): %w", s, err)
		}
		if port == "" {
			return nil, fmt.Errorf("cf_http: bind %q: port is required", s)
		}
		canon := net.JoinHostPort(host, port)
		if _, ok := seen[canon]; ok {
			return nil, fmt.Errorf("cf_http: duplicate bind %q", canon)
		}
		seen[canon] = struct{}{}
